...
def get_members():
    members = the_family.get_all_members()
    return jsonify(members), 200

@app.route('/member/<int:member_id>', methods=['GET'])
def get_one_member(member_id):
    member = the_family.get_member(member_id)
...
def add_member():
    ...
    member = {
        "id": body_id or the_family._generateId(),
    ...
    return jsonify('Usuario agregado exitosamente'), 200
